...
def skeletonize(pcd, nparts=2):
    ...
    for t in ts:
        ...
        bone = od.geometry.PointCloud(points=od.utility.Vector3dVector(np.array(cg)))
        bone.paint_uniform_color([1, 0, 0])
        meat = od.geometry.PointCloud(points=od.utility.Vector3dVector(points))
        meat.paint_uniform_color([0.2, 0.2, 0.2])
        od.visualization.draw_geometries([bone, meat])
...
if __name__ == "__main__":
    ...
    nboxes = 2 #Number of boxes in which to divide the bounding box of the given set of points
