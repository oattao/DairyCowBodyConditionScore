...
colors_view = { # iterate with `keys = list(colors)` and `colors[keys[0]]`
  "head"     : [1.0, 0.0, 0.0],
  "tail"   : [0.0, 1.0, 0.0],
  "left"    : [0.0, 0.0, 1.0],
  "right"  : [1.0, 1.0, 0.0],
...
def visualize_view(verts, labels, view, plot_keypoints=True, plot_distance=True):
    if labels.ndim == 1:
        labels = np.expand_dims(labels, axis=-1)
    np_colors = np.tile(1.0, verts.shape)
    # update global colormap
    color_to_add = np.asarray(colors_view[view]) - np.asarray(colors_view["white"])
    values_to_add = np.asarray([labels[:,0], labels[:,0], labels[:,0]]).transpose()
    if plot_distance:
        np_colors = np_colors + values_to_add * color_to_add
    else:
        np_colors = np_colors
    ...
    # add spheres to plot
    distance_predicted = np.asarray(labels[:,0])
    max_value = np.argmax(distance_predicted)
